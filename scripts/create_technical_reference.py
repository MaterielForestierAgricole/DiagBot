#!/usr/bin/env python3
"""Crée un brouillon Markdown vide à partir du modèle de référence technique."""
...
def arguments() -> argparse.Namespace:
    ...
    parser.add_argument("--title", required=True, help="titre du sujet")
...
def main() -> int:
    ...
    try:
        ...
        if not MACHINE_DIRECTORY_RE.fullmatch(folder):
            raise DiagBotError("Nom de dossier machine ou famille invalide.")
        ...
        keywords = sorted({value.strip() for value in args.keywords if value.strip()})
        if not keywords:
            raise DiagBotError("Au moins un mot-clé est requis.")

        directory = KNOWLEDGE_ROOT / "drafts" / "technical_reference" / folder
        destination = directory / filename
        if destination.exists():
            raise DiagBotError(f"Le document existe déjà : {relative(destination)}")
        directory.mkdir(parents=True, exist_ok=True)

        template = (REPOSITORY_ROOT / "templates" / "technical_reference.md").read_text(
            encoding="utf-8"
        )
        replacements = {
            ...
            "[KEYWORDS]": "\n".join(f"  - {yaml_string(value)}" for value in keywords),
